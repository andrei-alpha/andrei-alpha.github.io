...
var combinationLock = {
	...
	check: function() {
		if (this.combination1 === parseInt(this.wheels.join(''))) {
			this.locked = false;
			this.secret_message = "Next clue in a palm tree :)";
		} else if (this.combination2 === parseInt(this.wheels.join(''))) {
			this.locked = false;
			this.secret_message = "Were dreams are formed lies your gift :)";
		} else {
			this.locked = true;
		}
	},
...
function checkLock() {
	...
	if (combinationLock.locked === false) {
		...
		// Show the secret
		console.log(combinationLock.secret_message)
		$('#secret span').text(combinationLock.secret_message)
		var spanWidth = $('#secret span').width();
		$('#secret').animate( { width: spanWidth }, 4000 );
	} else {
